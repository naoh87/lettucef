...
trait SortedSetCommands[F[_], K, V] extends CommandsDeps[F, K, V] with SortedSetCommandsF[F, K, V] {
  ...
  def zlexcount(key: K, range: RedisRange[V]): F[Long] =
    JF.toSync(underlying.zlexcount(key, range.toJavaAuto)).map(Long2long)
  ...
  def zrangebylex(key: K, range: RedisRange[V]): F[Seq[V]] =
    JF.toSync(underlying.zrangebylex(key, range.toJavaAuto)).map(_.asScala.toSeq)
  
  def zrangebylex(key: K, range: RedisRange[V], limit: Limit): F[Seq[V]] =
    JF.toSync(underlying.zrangebylex(key, range.toJavaAuto, limit)).map(_.asScala.toSeq)
  ...
  def zrangestore(dstKey: K, srcKey: K, range: RedisRange[Long]): F[Long] =
    JF.toSync(underlying.zrangestore(dstKey, srcKey, range.toJavaAuto)).map(Long2long)
  
  def zrangestorebylex(dstKey: K, srcKey: K, range: RedisRange[V], limit: Limit): F[Long] =
    JF.toSync(underlying.zrangestorebylex(dstKey, srcKey, range.toJavaAuto, limit)).map(Long2long)
  ...
  def zremrangebylex(key: K, range: RedisRange[V]): F[Long] =
    JF.toSync(underlying.zremrangebylex(key, range.toJavaAuto)).map(Long2long)
  ...
  def zrevrangebylex(key: K, range: RedisRange[V]): F[Seq[V]] =
    JF.toSync(underlying.zrevrangebylex(key, range.toJavaAuto)).map(_.asScala.toSeq)
  
  def zrevrangebylex(key: K, range: RedisRange[V], limit: Limit): F[Seq[V]] =
    JF.toSync(underlying.zrevrangebylex(key, range.toJavaAuto, limit)).map(_.asScala.toSeq)
  ...
  def zrevrangestore(dstKey: K, srcKey: K, range: RedisRange[Long]): F[Long] =
    JF.toSync(underlying.zrevrangestore(dstKey, srcKey, range.toJavaAuto)).map(Long2long)
  
  def zrevrangestorebylex(dstKey: K, srcKey: K, range: RedisRange[V], limit: Limit): F[Long] =
    JF.toSync(underlying.zrevrangestorebylex(dstKey, srcKey, range.toJavaAuto, limit)).map(Long2long)
